      ...
      try {
        ...
        if (firstWord === '!parar' || firstWord === '!stop' || firstWord === '!srparar' || firstWord === '!srstop') {
          ...
          if (isModOrBroadcaster) {
            const res = songRequest.stopSong(this.currentChannel, username);
            this.sendMessage(this.currentChannel, res.message);
          } else {
            this.sendMessage(this.currentChannel, `@${username}, solo moderadores y el streamer pueden detener la música.`);
          }
          return;
        }
